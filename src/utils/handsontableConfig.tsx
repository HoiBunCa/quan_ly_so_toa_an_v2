...
function multiLineTextRenderer(instance: any, td: HTMLElement, row: number, col: number, prop: string, value: any, cellProperties: Handsontable.CellProperties) {
  ...
  // Custom formatting for 'thong_tin_nguoi_khoi_kien', 'thong_tin_nguoi_bi_kien', 'thong_tin_nguoi_co_quyen_loi_va_nghia_vu_lien_quan'
  ...
  } else if (prop === 'thong_tin_nguoi_tham_gia_hoa_giai') { // NEW: Formatting for participant info
    if (typeof value === 'string') {
      const lines = value.split('\n');
      let formattedValue = '';
      if (lines.length >= 1 && lines[0]) {
  ...
  } else if (prop === 'thong_tin_ban_an_quyet_dinh_phuc_tham') { // NEW: Formatting for combined number/date/summary text for TO_TUNG
    if (typeof value === 'string') {
      const lines = value.split('\n');
      let formattedValue = '';
      if (lines.length >= 1 && lines[0]) {
